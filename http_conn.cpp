...
void setnonblocking(int fd){
    ...
    int new_flag=old_flg|O_NONBLOCK;
...
//修改文件描述符，重置socket上的EPOLLONESHOT事件，保证下一次可读时能被触发
...
void http_conn::init(){
    ...
    m_check_state =CHECK_STATE_REQUESTLING;    // 初始状态为检查请求行
    m_linger = false;       // 默认不保持链接  Connection : keep-alive保持连接

    m_check_state=CHECK_STATE_REQUESTLING;//初始化状态信息为解析请求首行
    m_checked_index=0;//
    m_start_line=0;
    m_read_idx=0;
    m_method=GET;
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text){
        ...
        *m_version++='\0';
        ...
        m_check_state=CHECK_STATE_HEADER;//主状态机检查状态变成检查请求
        return NO_REQUEST;
}
...
http_conn::LINE_STATUS http_conn::parse_line(){
    ...
    for(;m_checked_index<m_read_idx;++m_checked_index){
        ...
        if(temp=='\r'){
            ...
            else if(m_read_buf[m_checked_index+1]=='/n'){
